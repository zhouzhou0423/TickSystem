...
function stopDefault(a) {
	...
	var b = f.cookie = function(q, p, v) {
		if (arguments.length > 1 && !f.isFunction(p)) {
			...
			return (document.cookie = [d(q), "=", h(p), v.expires ? "; expires=" + v.expires.toUTCString() : "", v.path ?
				"; path=" + v.path : "", v.domain ? "; domain=" + v.domain : "", v.secure ? "; secure" : ""
			].join(""))
		}
		var w = q ? undefined : {},
...
(function(a) {
					...
					if (V.extraData) {
						for (var ak in V.extraData) {
							if (V.extraData.hasOwnProperty(ak)) {
								if (f.isPlainObject(V.extraData[ak]) && V.extraData[ak].hasOwnProperty("name") && V.extraData[ak].hasOwnProperty(
										"value")) {
									aq.push(f('<input type="hidden" name="' + V.extraData[ak].name + '">').val(V.extraData[ak].value).appendTo(
										L)[0])
								} else {
	...
	f.fn.ajaxForm = function(g) {
		...
		if (g.delegation) {
			f(document).off("submit.form-plugin", this.selector, b).off("click.form-plugin", this.selector, a).on(
				"submit.form-plugin", this.selector, g, b).on("click.form-plugin", this.selector, g, a);
			return this
		}
	...
	f.fieldValue = function(g, o) {
		...
		if (o && (!j || g.disabled || u == "reset" || u == "button" || (u == "checkbox" || u == "radio") && !g.checked ||
				(u == "submit" || u == "image") && g.form && g.form.clk != g || w == "select" && g.selectedIndex == -1)) {
			return null
		}
		if (w == "select") {
	...
	var c = function(e, g) {
		...
		m.prototype = {
			...
			_calcHeight: function() {
				...
				var x = "." + n.jsClass + " ." + n.navClass + "-" + this.index + ".opened{max-height:" + z + "px !important} ." +
					n.jsClass + " ." + n.navClass + "-" + this.index + ".opened.dropdown-active {max-height:9999px !important}";
				if (w.styleSheet) {
...
! function(a) {
	a.fn.Huispinner = function(c, e) {
		...
		this.each(function() {
			var g = a(this);
			var k = '<div class="spinner">' + '<a class="subtract" href="javascript:void(0)"><i>-</i></a>' +
				'<input class="amount input-text" value="' + c.value + '" autocomplete="off">' +
				'<a class="add" href="javascript:void(0)"><i>+</i></a>' + "</div>";
			g.append(k);
...
! function(a) {
	a.fn.Huitags = function(c) {
		...
		this.each(function() {
			var j = a(this);
			var n = '<div class="Huitags-wraper">' + '<div class="Huitags-editor cl"></div>' +
				'<div class="Huitags-input-wraper">' + '<input type="text" class="input-text Huitags-input" maxlength="' + c.maxlength +
				'" value="">' + "</div>" + '<div class="Huitags-list">' +
				'<div class="Huitags-notag" style="display:none">暂无常用标签</div>' + '<div class="Huitags-has"></div>' + "</div>" +
				'<input type="hidden" class="Huitags-val" name="" value="' + c.value + '">' + "</div>";
			j.append(n);
			...
			d.hover(function() {
				...
				e = setTimeout(function() {
					...
					if (n == "") {
						...
						var j = '<div id="preview-wraper" style="position: absolute;width:' + b.bigImgWidth +
							"px;height:auto;top:" + r + "px;right:" + h + ";left:" + s + '">' + '<img src="' + f + '" width="' + b.bigImgWidth +
							'">' + "</div>";
						a("body").append(j);
						var i = new Image();
						i.src = n;
...
! function(a) {
	a.Huimodalalert = function(c, b) {
		...
		if (b == 0 || typeof(b) == "undefined") {
			a(document.body).append('<div id="modal-alert" class="modal modal-alert radius">' +
			...
			a(document.body).append('<div id="modal-alert" class="modal modal-alert radius">' +
				'<div class="modal-alert-info">' + c + "</div>" + "</div>");
			a("#modal-alert").fadeIn();
...
! function(d) {
	...
	b.prototype.hide = function(f) {
		...
		d.support.transition && this.$element.hasClass("fade") ? this.$element.one("bsTransitionEnd", d.proxy(this.hideModal,
			this)).emulateTransitionEnd(b.TRANSITION_DURATION) : this.hideModal()
	};
	...
	b.prototype.backdrop = function(i) {
		...
		if (this.isShown && this.options.backdrop) {
			...
			this.$backdrop = d('<div class="modal-backdrop ' + f + '" />').prependTo(this.$element).on("click.dismiss.bs.modal",
				d.proxy(function(j) {
			...
			if (!this.isShown && this.$backdrop) {
				...
				d.support.transition && this.$element.hasClass("fade") ? this.$backdrop.one("bsTransitionEnd", g).emulateTransitionEnd(
					b.BACKDROP_TRANSITION_DURATION) : g()
			} else {
				if (i) {
					i()
				}
...
! function(h) {
	...
	h(document).on("click.bs.dropdown.data-api", d).on("click.bs.dropdown.data-api", ".dropdown form", function(i) {
			i.stopPropagation()
		}).on("click.bs.dropdown.data-api", b, a.prototype.toggle).on("keydown.bs.dropdown.data-api", b, a.prototype.keydown)
...
! function(d) {
	...
	c.prototype.show = function() {
		...
		if (this.hasContent() && this.enabled) {
			...
			var l = typeof this.options.placement == "function" ? this.options.placement.call(this, m[0], this.$element[0]) :
			...
			if (u) {
				...
				l = l == "bottom" && r.bottom + n > h.bottom ? "top" : l == "top" && r.top - n < h.top ? "bottom" : l == "right" &&
					r.right + f > h.width ? "left" : l == "left" && r.left - f < h.left ? "right" : l;
			...
			d.support.transition && this.$tip.hasClass("fade") ? m.one("bsTransitionEnd", g).emulateTransitionEnd(c.TRANSITION_DURATION) :
				g()
		}
	};
	...
	c.prototype.hide = function(j) {
		...
		i.removeClass("in");
		d.support.transition && this.$tip.hasClass("fade") ? i.one("bsTransitionEnd", f).emulateTransitionEnd(c.TRANSITION_DURATION) :
			f();
...
! function(d) {
	...
	c.prototype.setContent = function() {
		...
		g.find(".popover-content").children().detach().end()[this.options.html ? (typeof e == "string" ? "html" : "append") :
			"text"](e);
...
! function(e) {
	...
	b.prototype.close = function(j) {
		...
		e.support.transition && h.hasClass("fade") ? h.one("bsTransitionEnd", f).emulateTransitionEnd(b.TRANSITION_DURATION) :
			f()
	};
	...
	(function(e) {
		...
		a.prototype = {
			...
			setValue: function(k, i) {
				...
				if (this.diff > 0) {
					this.percentage = [(this.options.value[0] - this.options.min) * 100 / this.diff, (this.options.value[1] -
						this.options.min) * 100 / this.diff, this.options.step * 100 / this.diff]
				} else {
			...
			_calculateValue: function() {
				...
				if (this.options.range) {
					...
					if (this.percentage[0] !== 0) {
						g[0] = (Math.max(this.options.min, this.options.min + Math.round((this.diff * this.percentage[0] / 100) /
							this.options.step) * this.options.step));
						g[0] = this._applyPrecision(g[0])
					}
					if (this.percentage[1] !== 100) {
						g[1] = (Math.min(this.options.max, this.options.min + Math.round((this.diff * this.percentage[1] / 100) /
							this.options.step) * this.options.step));
						g[1] = this._applyPrecision(g[1])
					}
				} else {
					g = (this.options.min + Math.round((this.diff * this.percentage[0] / 100) / this.options.step) * this.options
						.step);
	...
	i.prototype = {
		...
		update: function() {
			...
			if (arguments && arguments.length && (typeof arguments[0] === "string" || arguments[0] instanceof Date)) {
				...
				j = (this.isInput ? this.element.val() : this.element.find("input").val()) || this.element.data("date") || this
					.initialDate;
